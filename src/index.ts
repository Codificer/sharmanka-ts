...
const Sharmanka:SharmankaType = {
	...
	onPlay: function onPlay(callback) {
		try {
			if (_instanceof(callback, Function)) this.node.addEventListener('playing', function removingListener() {
	...
	onPause: function onPause(callback) {
		try {
			if (_instanceof(callback, Function)) this.node.addEventListener('pause', function removingListener() {
				return callback(removingListener);
			});
		} catch (e) {
			console.error(e);
	...
	onStart: function onStart(callback) {
		try {
			if (_instanceof(callback, Function)) this.node.addEventListener('playing', function removingListener() {
				if (this.node.currentTime === 0) callback(removingListener);
			});
		} catch (e) {
			console.error(e);
		}
	...
	onEnd: function onEnd(callback) {
		try {
			if (_instanceof(callback, Function)) this.node.addEventListener('ended', function removingListener() {
				return callback(removingListener);
			});
		} catch (e) {
			console.error(e);
	...
	onLoad: function onLoad(callback) {
		try {
			if (_instanceof(callback, Function)) this.node.addEventListener('loadedmetadata', function removingListener() {
				return callback(removingListener);
			});
		} catch (e) {
			console.error(e);
	...
	onTick: function onTick(callback) {
		try {
			if (_instanceof(callback, Function)) this.node.addEventListener('timeupdate', function removingListener() {
				return callback(removingListener);
			});
		} catch (e) {
			console.error(e);
	...
	onBuffer: function onBuffer(callback) {
		try {
			if (_instanceof(callback, Function)) this.node.addEventListener('progress', function removingListener() {
				return callback(removingListener);
			});
		} catch (e) {
			console.error(e);
	...
	onLoadError: function onLoadError(callback) {
		if (_instanceof(callback, Function)) this.node.addEventListener('error', function removingListener() {
			return callback(removingListener);
		});
	},
	onError: function onError(callback) {
		if (_instanceof(callback, Function)) this.node.addEventListener('abort', function removingListener() {
			return callback(removingListener);
		});
	},
	removeEvent: function removeEvent(eventName, callback) {
		if (_instanceof(callback, Function)) return this.node.removeEventListener(eventName, callback);
	},
	...
	volume: function volume(value) {
			...
			if (typeof value === 'number' && (value < 0 || value > 1)) console.error('Volume value should be a positive number between 0 and 1');
	},
	mute: function mute() {
		try {
	...
	seek: function seek(value) {
			...
			if(typeof value !== 'number') console.error('SeekValue should be a number');
			if(typeof value === 'number' && (value < 0 || value > this.duration)) return console.error('Seek Value should be more then 0 and less then track duration')
	},
	setTrack: function setTrack(value) {
			Sharmanka.buffered = 0;
			Sharmanka.currentTime = 0;
			Sharmanka.duration = 0;
			if (typeof value === 'string' && value.length > 0) this.node.src = value;
